 ...
 * @brief 1d col block distribution
 * @version 0.1
 * @date 2022-06-23
 *
...
void gaussEliminationMPI(float a[][n], int argc, char *argv[])
{
    ...
    int c_start; // start col of distributed task
    int c_end;   // end col of distributed task
    int c_stride;
    ...
    stride1 = n / num + 1;
    stride2 = n / num;
    MPI_Type_vector(n, stride1, n, MPI_FLOAT, &C1);
    MPI_Type_commit(&C1);
    MPI_Type_vector(n, stride2, n, MPI_FLOAT, &C2);
    MPI_Type_commit(&C2);
    strides = new int[num + 1];
    strides[0] = 0;
    for (int i = 1; i <= num; i++)
    {
        if ((i - 1) < n % num)
        {
            strides[i] = stride1 + strides[i - 1];
        }
        else
        {
            strides[i] = stride2 + strides[i - 1];
        }
    }
    // for(int i=0;i<num;i++)
    // {
    ...
    c_start = myid * (n / num) + ((myid < n % num) ? myid : n % num);
    c_stride = (myid < n % num) ? stride1 : stride2;
    c_end = c_start + c_stride - 1;
    // cout<<"myid: "<<myid<<" cs: "<<c_start<<" ce: "<<c_end<<" stride: "<<c_stride<<endl;
    ...
    if (myid == 0)
    {
        ...
        for (int i = 1; i < num; i++)
        {
            ...
            if (i < n % num)
            {
                MPI_Send(&a[0][i_start], 1, C1, i, 0, MPI_COMM_WORLD);
            }
            else
            {
                MPI_Send(&a[0][i_start], 1, C2, i, 0, MPI_COMM_WORLD);
            }
        }
    }
    else
    {
        if (myid < n % num)
        {
            MPI_Recv(&a[0][c_start], 1, C1, 0, 0, MPI_COMM_WORLD, &status);
            // printMatrix(a);
        }
        else
        {
            MPI_Recv(&a[0][c_start], 1, C2, 0, 0, MPI_COMM_WORLD, &status);
            // printMatrix(a);
        }
    }
    ...
    for (int k = 0; k < n; k++)
    {
        // find the process having a[k][k] and broadcast to others
        int root = -1;
        int root_start, root_end;
        do
        {
            root++;
            root_start = root * (n / num) + ((root < n % num) ? root : n % num);
            root_end = root_start + (n / num) + ((root < n % num) ? 1 : 0) - 1;
        } while (k < root_start || k > root_end);
        // cout<<"k = "<<k<<" root = "<<root<<endl;
        MPI_Bcast(&a[k][k], 1, MPI_FLOAT, root, MPI_COMM_WORLD);

        // each corresponding one does division and elimination work individually
        if (myid >= root)
        {
            for (int j = max(c_start, k + 1); j <= c_end; j++)
            {
                a[k][j] /= a[k][k];
            }
            a[k][k] = 1;
    ...
    if (myid != 0)
    {
        if (myid < n % num)
        {
            MPI_Send(&a[0][c_start], 1, C1, 0, myid, MPI_COMM_WORLD);
        }
        else
        {
            MPI_Send(&a[0][c_start], 1, C2, 0, myid, MPI_COMM_WORLD);
        }
    }
    else
    {
        for (int i = 1; i < num; i++)
        {
            if (i < n % num)
            {
                MPI_Recv(&a[0][strides[i]], 1, C1, i, i, MPI_COMM_WORLD, &status);
            }
            else
            {
                MPI_Recv(&a[0][strides[i]], 1, C2, i, i, MPI_COMM_WORLD, &status);
            }
        }
    }
    // if (myid == 0)
