...
size_t eliminatantSize;
size_t eliminatorSize;
...
void init()
{
    ...
    eliminatantSize = wndSize1 * wrdLen * sizeof(int);
    eliminatorSize = wndSize * wrdLen * sizeof(int);
    ...
    //cudaMemPrefetchAsync(eliminatant, eliminatantSize, cudaCpuDeviceId);
    //cudaMemPrefetchAsync(eliminator, eliminatorSize, cudaCpuDeviceId);
...
void gaussian()
{
    ...
    for (int i = 0; i < wndSize1; i++)
    {
        ...
        //cudaMemPrefetchAsync(eliminatant, eliminatantSize, deviceId);
        //cudaMemPrefetchAsync(eliminator, eliminatorSize, deviceId);
