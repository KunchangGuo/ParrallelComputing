...
void gaussian()
{
    ...
    {
        MPI_Bcast(tmp, wrdLen, MPI_INT, (i - 1) / np, MPI_COMM_WORLD);
        BitManager tmpManager;
        buildBitManager(tmp, wrdLen, &tmpManager);
        // cout << "tmp lc: " << tmpManager.lftCol << endl;
        ...
        if (myid >= i / np)
        {
            int j = (myid == i / np) ? i % np : 0;
            // cout << "i = " << i << " myid = " << myid << " bingo = "<<(myid == i/np ? 1 : 0)<<" j = "<<j<<endl;
            for (; j < np; j++)
            {
                while (subManager[j].lftCol != -1 && eliminatorManager[subManager[j].lftCol].lftCol != -1)
                {
                    // cout << "myid = " << myid << endl
                    //      << toString(sub + wrdLen * j, subManager + j) << endl
                    //      << toString(eliminator + wrdLen * subManager[j].lftCol, eliminatorManager + subManager[j].lftCol) << endl;
                    xorBitmap(sub + wrdLen * j, eliminator + wrdLen * subManager[j].lftCol, subManager + j, eliminatorManager + subManager[j].lftCol);
                    // cout << "after xor: " << toString(sub + wrdLen * j, subManager + j) << endl;
                }
            }
        }
